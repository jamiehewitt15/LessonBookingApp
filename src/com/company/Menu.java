...
public class Menu {
    ...
    private String[] menuOptions = {        "Book Lesson", "Enter review",
    ...
    private void reviewLesson(Schedule schedule){
    ...
    private void changeLesson(Schedule schedule, CustomerList customerList){
        ...
        int reBookChoice = cancelLesson(deleteOptions, deleteChoice, schedule);
        switch (reBookChoice){
            case 1:
            ...
            for (int j = 0; j < schedule.scheduleArray[i].getLessonStudents().length; ++j) {
                for (int k = 0; k < schedule.lessonCapacity; ++k) {
                    if (schedule.scheduleArray[i].getLessonStudents(j, k) == customerList.currentUser.getName()){
                        System.out.println(schedule.scheduleArray[i].getLessonName() + " on " + schedule.getOpenDays(j) + " at " + schedule.scheduleArray[i].getLessonTime() + ":00");// Print lessons
    ...
    private int cancelLesson(ArrayList<int[]> options, int choice, Schedule schedule){
